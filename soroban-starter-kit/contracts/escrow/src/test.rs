...
fn create_token_contract<'a>(env: &Env) -> Address {
    ...
    env.register_contract_wasm(None, include_bytes!("../../token/target/wasm32-unknown-unknown/release/soroban_token_template.wasm"))
}
...
fn test_initialize_escrow() {
    ...
    let token_contract = Address::generate(&env); // Mock token contract
    let amount = 1000i128;
...
fn test_fund_wrong_state() {
    ...
    // Initialize, fund, then try to fund again
    client.initialize(&buyer, &seller, &arbiter, &token_contract, &amount, &deadline);
    client.fund();
